...
### Current Status
- **Last completed:** TEAM-tier Phase D (UI). New components: `components/dashboard/BranchSwitcher.tsx` (URL `?branchId=` sync, hidden when ≤1 branch), `components/settings/TeamBranchesSection.tsx` (enable card + branch list + sharing toggles, gated to TEAM via inline plan check), `components/settings/PermissionMatrix.tsx` (collapsible override panel; toggling back to preset removes the override key). New endpoint: `PATCH /api/branches/settings` for the two cross-branch sharing flags. `DashboardShell` now takes `sessionUser: { role, officeMemberRole, permissionsOverride }` instead of `role`, plus a `multiBranchEnabled` flag fed from the layout. `Sidebar`/`MobileBottomNav`/`MobileMoreSheet`/`QuickCreateSheet`/`SearchDialog` all switched from `managerOnly: boolean` to `requiresCapability?: OfficeCapability` and resolve gates via `canOfficeDo(sessionUser, ...)`. `lib/validations/agent.ts`: `createAgentSchema`/`updateAgentSchema` extended with `officeMemberRole`, `permissionsOverride` (sparse Record), `branchId`. `app/api/agents/route.ts` POST persists the new fields; `app/api/agents/[id]/route.ts` PATCH writes branch via `branch.connect/disconnect` and serializes override via `Prisma.JsonNull`. `AgentForm` shows the staff-role dropdown + permission matrix on TEAM, and the branch dropdown when multi-branch is enabled — feeds both into the request body. **Intentionally left alone:** `Topbar`/`SubscriptionBanner`/`ShareLinksPanel`/`DayEventsPanel` keep `role` as prop (their remaining `role === "MANAGER"` checks are semantic role identity rather than office capabilities; calendar rules preserved per plan). Server-page-level branch filtering (files/customers/KPI queries reading `?branchId` query param) is deferred — the switcher is wired to URL but list pages don't yet read it; that's incremental work on top of `lib/branch-scope.ts`.

- **Previously completed:** TEAM-tier Phase C (gate refactor). Swapped ~55 `role !== "MANAGER"` / `role === "MANAGER"` checks to `canOfficeDo(session.user, capability)` across server code. Capability mapping: `/api/agents/*` → `manageAgents`; `/api/contracts GET` + `/api/contracts/[id] GET` + `schedule-sms GET` → `viewContracts`; `/api/contracts POST` + `schedule-sms POST/DELETE` + `/contracts/new` → `finalizeContract` (with legacy `canFinalizeContracts` DB fallback for pre-migration agents); `/api/files/[id]/status` + `/api/share-links/[id]` → `deleteFile`; `/api/files/[id]/agents` → `assignFile`; `/api/files/[id]/share-links` agent-attribution → `assignFile` (BRANCH_MANAGER + Owner take the "assign any office agent" branch, plain agents take the "self + must be assigned" branch); `/api/files/[id]` activity-log include → `viewActivityLog`; `/api/messages/*` → `sendBulkSms`; `/api/referral/*` + `/api/settings/*` + `/api/settings/logo` + `/api/subscription/usage` + `/api/payments/request` → `manageOffice`; `/api/branches/*` + `/api/branches/enable` → `manageBranches`; `/api/search` split into `canSearchAgents` (manageAgents) + `canSearchContracts` (viewContracts). Dashboard page redirects: `/reports/*` → `viewReports`, `/contracts/*` → `viewContracts`, `/contracts/new` → `finalizeContract`+DB-fallback, `/agents/*` → `manageAgents`, `/messages` → `sendBulkSms` (client fetches the extended session fields), `/referral` + `/settings` → `manageOffice`. Server-component UI in `dashboard/page.tsx` and `files/[id]/page.tsx` precomputes `canManageAgents`/`canViewReports`/`canAssignFile`/`canDeleteFile`/`canViewActivityLog` flags via `canOfficeDo` and uses them for conditional rendering. **Intentionally left alone:** `/api/calendar/events*` + calendar UI (plan: "existing rules preserved"); `app/(dashboard)/layout.tsx` Owner-onboarding flow; `dashboard/page.tsx` Owner plan-upgrade notification; `crm/[id]/page.tsx` admin-perspective `isManager`; `files/route.ts` managerIsAgent auto-assign logic; `lib/office-permissions.ts` + `lib/branch-scope.ts` implementation sites; pure-UI client components receiving `role` as a prop (Topbar, SubscriptionBanner, ShareLinksPanel, DayEventsPanel — these belong to Phase D's capability-aware sidebar & Staff page refactor).
- **Up next:** Phase E — wire the `?branchId` query param into list-page server queries (files, customers, KPI widgets) using `lib/branch-scope.ts`'s `buildBranchFilter`, and gate the multi-branch enable endpoint behind `PLAN_FEATURES[plan].hasMultiBranch === true`. Add a new `hasCustomStaffRoles` plan flag (TEAM-only) to gate the staff role dropdown in the form. Still outstanding from previous iterations: add `* * * * * curl -s -X POST -H "x-cron-secret: $CRON_SECRET" http://localhost:3000/api/cron/fire-calendar-reminders` to VPS crontab, and manual test the calendar reminder ding + ticket toast together.
- **Total tests:** 716 passing, 0 failing (49 test files). Typecheck: 43 pre-existing errors (unchanged — zero new errors introduced by Phase D).
