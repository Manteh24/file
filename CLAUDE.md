...
### Current Status
- **Last completed:** TEAM-tier Phase E (branch query param + plan flag gates). New helper `resolveBranchScope(user, office, entity, requestedBranchId)` in `lib/branch-scope.ts` overlays `buildBranchFilter`'s visibility filter with the `?branchId` from the BranchSwitcher: branch-scoped users can never widen scope (visibility wins), but MANAGER / `viewAllBranches` users can narrow to one branch via the switcher. Wired into `app/(dashboard)/files/page.tsx` (file list), `app/(dashboard)/crm/page.tsx` (customer list), and `app/(dashboard)/dashboard/page.tsx` (active files count, customer count, file status groupBy — contracts/team count stay office-wide since they have no branchId). Each page fetches `office.{multiBranchEnabled, shareFilesAcrossBranches, shareCustomersAcrossBranches}` and merges the resulting `{ branchId? }` fragment into the existing `where` clause. `POST /api/branches/enable` now uses `PLAN_FEATURES[sub.plan].hasMultiBranch` instead of an inline `sub.plan !== "TEAM"` check (semantically identical today, but follows the plan-feature-flag pattern). New `hasCustomStaffRoles: boolean` flag added to `PLAN_FEATURES` (`true` for TEAM, `false` for FREE/PRO); `AgentForm` swapped from `plan === "TEAM"` to `PLAN_FEATURES[plan].hasCustomStaffRoles` for the staff role dropdown gate.
...
- **Up next:** TEAM-tier expansion code is complete (Phases A1–A3, B, C, D, E all merged). Remaining work is **manual smoke testing** (per plan §Verification): (1) single-office TEAM user — confirm nothing branch-related appears until enabled, (2) enable multi-branch → confirm HQ created and existing files/customers/agents stamped, (3) add a second branch + branch manager + agents, (4) toggle file sharing off → log in as Branch B agent → confirm HQ files hidden, (5) create a CUSTOM staff with only `viewReports` → confirm capability scoping, (6) PRO/FREE regression — Team & Branches tab shows only the upgrade CTA, agent creation unchanged. Also still outstanding from previous iterations: add `* * * * * curl -s -X POST -H "x-cron-secret: $CRON_SECRET" http://localhost:3000/api/cron/fire-calendar-reminders` to VPS crontab, and manual test the calendar reminder ding + ticket toast together. Deferred to v2 (per plan §Out of scope): per-branch billing, cross-branch file/customer transfer, multi-branch-per-user, branch-level activity log dashboards, geography-based lead routing.
- **Total tests:** 723 passing, 0 failing (49 test files — +7 new tests for `resolveBranchScope` in `__tests__/lib/branch-scope.test.ts`). Typecheck: 24 pre-existing errors (all in `__tests__/` or `components/files/FileMapView.tsx` — zero new errors introduced by Phase E).
