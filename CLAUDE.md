...
- **Up next:** Cluster C Phase 2 (Tier-1) is complete. Tier-2 follow-up is the next clean piece of work — wire the same `toastSuccess`/`toastError` pattern into the ~12 manager-write surfaces (Agent forms, Customer forms, Contract forms, Settings forms, Office profile, User profile), and the ~6 admin non-destructive writes (broadcast send, PlatformSetting editor, mid-admin CRUD). After that: Cluster D — replace `window.confirm()` with shadcn `AlertDialog` for destructive actions (now cheap since we have the toast layer). Still outstanding from prior iterations: TEAM-tier multi-branch manual smoke tests, calendar-reminder cron entry on VPS (`* * * * * curl -s -X POST -H "x-cron-secret: $CRON_SECRET" http://localhost:3000/api/cron/fire-calendar-reminders`), and manual test of the calendar ding + ticket toast together. Deferred to v2: per-branch billing, cross-branch file/customer transfer, multi-branch-per-user, branch-level activity log dashboards, geography-based lead routing.
