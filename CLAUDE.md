...
### Current Status
- **Last completed:** Support ticket submission now shows a fade-in success toast before redirecting. In `app/(dashboard)/support/new/page.tsx`: added `showSuccessToast` state, a fixed top-center emerald pill with `CheckCircle2` icon + «تیکت با موفقیت ارسال شد», and a 1.4s delay before `router.push` so the toast actually registers. `isSubmitting` stays true during the delay to prevent double-submits. Uses existing tailwindcss-animate utilities — no new dependency, no new shared component.
- **Up next:** Still outstanding from previous iteration — add `* * * * * curl -s -X POST -H "x-cron-secret: $CRON_SECRET" http://localhost:3000/api/cron/fire-calendar-reminders` to VPS crontab, and run `npx prisma migrate deploy` for the `reminderFiredAt` column. Manual test the calendar reminder ding + the new ticket toast together.
