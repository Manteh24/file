...
| — | UX Cluster B — Activation polish (audit findings #4, #6, #8 + adjacent F-2.3 / F-2.5). New `components/dashboard/FirstRunChecklist.tsx` (MANAGER-only, 2-step: file → share link, auto-hides on completion, Linear-style × dismiss persisted in `localStorage["dashboard_checklist_dismissed"]`, `/guide` link in footer). `app/(dashboard)/dashboard/page.tsx` extended `Promise.all` with `db.propertyFile.count({where:{officeId}})` + `db.shareLink.count({where:{file:{officeId}}})` (gated `isManager` to keep AGENT query path identical). `components/files/PhotoGallery.tsx` migrated from `fetch()` to `XMLHttpRequest` for upload-progress events: per-image placeholder card with `Skeleton` + 0–100% progress bar (advances on `xhr.upload.progress`), then "در حال پردازش..." spinner + `Loader2` during the server Sharp window (we cannot observe). Inline `setUploadError`/`<p>` removed in favor of `toastError` from `lib/toast.ts`. `components/files/FileForm.tsx` — chip-row hint "عکس · قیمت · امکانات · توضیحات" added under the existing "تکمیل فایل" pill in collapsed/create mode; pill wrapper goes sticky on mobile (`sticky bottom-[calc(4rem+env(safe-area-inset-bottom))] z-30 md:static`) so it clears the fixed `MobileBottomNav`. `components/dashboard/SubscriptionBanner.tsx` near_expiry copy reframed from anxious ("اشتراک در حال انقضا") to gift-framed ("همچنان از همه قابلیت‌ها استفاده کنید"); grace + locked states untouched (post-expiry warnings stay). `TrialActivationBanner.tsx` left as-is (already gift-framed teal, audit's red/orange complaint was misattributed). | ✅ | — |

### Current Status
- **Last completed:** UX Cluster B — Activation. Audit findings #4 (mobile collapse-toggle invisible), #6 (photo upload no progress), #8 (first-run dashboard CTA) all resolved, plus adjacent F-2.3 (banner copy) and F-2.5 (`/guide` link). Five files touched: `FirstRunChecklist.tsx` (new), `dashboard/page.tsx`, `PhotoGallery.tsx`, `FileForm.tsx`, `SubscriptionBanner.tsx`. No new server routes, no migration. Tests: 747/747 still passing (50 files); typecheck stays at the 24 pre-existing baseline errors (all in `__tests__/` or `FileMapView.tsx`). Manual mobile + slow-3G smoke still pending (browser-only check).

- **Previously completed:** UX Cluster C Phase 3 + Cluster D Phase 1. **Cluster C Phase 3** wired toasts into all Tier-2 manager-write surfaces (AgentForm, CustomerForm, ContractForm, OfficeProfileForm, UserProfileForm, TeamBranchesSection, ReferralDashboard) and all Tier-3 admin non-destructive writes (broadcast send, AdminSettingsForm, MidAdminForm × 5 sub-forms, OfficeNotesPanel). All inline saved/error pills with redundant data were stripped (kept only contextual inline errors that point at specific form fields, e.g., MidAdminForm's rule-empty validation). **Cluster D Phase 1** replaced all four `window.confirm()` callsites — `ArchiveFileButton`, `SuspendReactivateButtons`, `ArchiveRestoreButtons`, `ContractSmsActions` cancel-schedule — with shadcn `AlertDialog`. The audit's #1 (🔴) and #3 (🟠) findings are now resolved; `docs/ux-audit.md` updated to reflect this.
...
- **Up next:** With Cluster B done (audit #4/#6/#8 resolved), the highest-impact remaining work is **Cluster A (Conversion: F-1.6 hero JTBD copy, F-1.11 register form length, F-1.28 social proof + landing dark-mode AA pass)** — this is the biggest revenue-side lever still open and targets the Top-10 finding #2 (★★★★★ register-form length) plus the §2.1 funnel drop-off. After Cluster A, the next options are **Cluster E (File-form polish: F-3a.4 phone tolerance, F-3a.11 main-contact validation timing, F-3a.15 SMS preview, F-3a.16 SMS silent success — note F-3a.6 photo progress is now done in Cluster B, F-3a.1 collapse-toggle is now done in Cluster B)** and **Cluster D Phase 2 (type-to-confirm for office archive, force-logout-all, delete-data)**. Still outstanding from prior iterations: TEAM-tier multi-branch manual smoke tests, calendar-reminder cron entry on VPS (`* * * * * curl -s -X POST -H "x-cron-secret: $CRON_SECRET" http://localhost:3000/api/cron/fire-calendar-reminders`), manual test of the calendar ding + ticket toast together, and Cluster B mobile + slow-3G photo-upload smoke. Deferred to v2: per-branch billing, cross-branch file/customer transfer, multi-branch-per-user, branch-level activity log dashboards, geography-based lead routing.
