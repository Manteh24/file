...
describe("sendSmsSchema", () => {
  ...
  it("accepts standard 11-digit Iranian mobile (09XXXXXXXXX)", () => {
    const result = sendSmsSchema.safeParse({ phone: "09123456789", message: "سلام", type: "share" })
    expect(result.success).toBe(true)
  })

  it("accepts 10-digit format without leading zero (9XXXXXXXXX)", () => {
    const result = sendSmsSchema.safeParse({ phone: "9123456789", message: "سلام", type: "share" })
    expect(result.success).toBe(true)
  })
  ...
  it("accepts all major Iranian operator prefixes (091, 093, 099, etc.)", () => {
    ...
    for (const phone of phones) {
      const result = sendSmsSchema.safeParse({ phone, message: "متن", type: "share" })
  ...
  it("rejects phone numbers starting with 08 (not a mobile prefix)", () => {
    const result = sendSmsSchema.safeParse({ phone: "08123456789", message: "سلام", type: "share" })
    expect(result.success).toBe(false)
    expect(result.error?.issues[0]?.message).toContain("معتبر نیست")
  })

  it("rejects phone numbers that are too short", () => {
    const result = sendSmsSchema.safeParse({ phone: "0912345", message: "سلام", type: "share" })
    expect(result.success).toBe(false)
  })

  it("rejects phone numbers that are too long", () => {
    const result = sendSmsSchema.safeParse({ phone: "091234567890", message: "سلام", type: "share" })
    expect(result.success).toBe(false)
  })

  it("rejects phone numbers containing letters", () => {
    const result = sendSmsSchema.safeParse({ phone: "0912ABC6789", message: "سلام", type: "share" })
    expect(result.success).toBe(false)
  })

  it("rejects landline numbers (021...)", () => {
    const result = sendSmsSchema.safeParse({ phone: "02112345678", message: "سلام", type: "share" })
    expect(result.success).toBe(false)
  })
  ...
  it("rejects an empty message", () => {
    const result = sendSmsSchema.safeParse({ phone: "09123456789", message: "", type: "share" })
    expect(result.success).toBe(false)
  ...
  it("rejects a message longer than 500 characters", () => {
    ...
    const result = sendSmsSchema.safeParse({ phone: "09123456789", message: longMessage, type: "share" })
    expect(result.success).toBe(false)
  ...
  it("accepts a message exactly 500 characters long", () => {
    ...
    const result = sendSmsSchema.safeParse({ phone: "09123456789", message: maxMessage, type: "share" })
    expect(result.success).toBe(true)
  })

  // ─── Type field validation ──────────────────────────────────────────────────

  it("accepts type=share", () => {
    const result = sendSmsSchema.safeParse({ phone: "09123456789", message: "سلام", type: "share" })
    expect(result.success).toBe(true)
  })

  it("accepts type=bulk", () => {
    const result = sendSmsSchema.safeParse({ phone: "09123456789", message: "سلام", type: "bulk" })
    expect(result.success).toBe(true)
  })

  it("rejects invalid type", () => {
    const result = sendSmsSchema.safeParse({ phone: "09123456789", message: "سلام", type: "custom" })
    expect(result.success).toBe(false)
  })
  ...
  it("rejects when phone field is missing", () => {
    const result = sendSmsSchema.safeParse({ message: "سلام", type: "share" })
    expect(result.success).toBe(false)
  })

  it("rejects when message field is missing", () => {
    const result = sendSmsSchema.safeParse({ phone: "09123456789", type: "share" })
    expect(result.success).toBe(false)
  })
})
